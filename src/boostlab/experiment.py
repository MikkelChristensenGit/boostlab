...
from boostlab.model import XGBClassificationModel, XGBRegressorModel
...
class XGBRegressorExperiment(Experiment):
    ...
    def run_pipeline(self) -> dict[str, Any]:
        ...
        self.model = XGBRegressorModel(params=self.params, num_rounds=self.num_rounds)
...
class XGBClassificationExperiment(Experiment):
    ...
    def __init__(
        ...
        self.random_state = random_state
        self.df: pd.DataFrame
        self.model: XGBClassificationModel
    ...
    def run_pipeline(self) -> dict[str, Any]:
        ...
        self.model = XGBClassificationModel(
            params=self.params, num_rounds=self.num_rounds
        )
        train_df = pd.concat([X_train, y_train.rename(self.target_col)], axis=1)
        self.model.fit(train_df, self.target_col)
        preds = self.model.predict(X_test)
